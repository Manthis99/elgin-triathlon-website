...
async function sendTicketEmail({ name, email, ticketCode, distance, quantity, amountTotal, currency }) {
  ...
  // Optional: route replies to a real inbox (the from-address is a
  // send-only domain with no mailbox). Set TICKET_REPLY_TO to your Gmail etc.
  const replyTo = process.env.TICKET_REPLY_TO;
...
async function fulfillCheckout(stripe, session) {
  ...
  if (email) {
    // Email failure (e.g. unverified Resend domain) must NOT fail the whole
    // webhook — the payment succeeded and the ticket is already recorded.
    // Log it and move on so Stripe doesn't retry in a loop.
    try {
      ...
      console.error("Ticket email failed (ticket still recorded):", err.message);
    }
  }
